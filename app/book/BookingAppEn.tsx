...
const NO_60_MIN_NOTE = "No available 60-minute interval for this date.";
const EMPTY_FIRST_FREE: FirstFreeMap = {
  any: null,
  daniel: null,
  elitsa: null,
};
const PACKAGE_PRICE_LABEL = "5-session package";
...
export default function BookingAppEn() {
  ...
  const setDurationSafe = (d: 30 | 60 | 90) => {
    ...
    try {
      ...
      if (duration === 60) {
        ...
        if (!any90) setNote("No available 90-minute interval for this date.");
      } else {
      ...
      setSlots([]);
      setError(e instanceof Error ? e.message : "Error while loading available times.");
    } finally {
      setLoading(false);
    }
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccessText(
        `Успешно bookхте час! ${fmtDateHeader(
          date
        )} • ${toHHMM(start)}–${toHHMM(end)} (${duration} мин)` +
          (therapist !== "any" ? ` • Therapist: ${tName}` : "")
      );
      ...
      setError(e instanceof Error ? e.message : "Booking error.");
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 shadow-sm">
            <div className="px-6 py-6">
              ...
              <div className="mt-4 rounded-lg border border-emerald-200 bg-white/70 p-4 text-emerald-900">
                <div>
                  <strong>Address:</strong> Sofia, Prof. Hristo Danov 19
                </div>
                <div>
                  <strong>Phone:</strong>{" "}
                  <a
          ...
          <div className="flex-1 rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col">
            <div className="p-4 flex-1">
              <h2 className="text-center text-[22px] font-semibold text-slate-900">
                Book an appointment by choosing a date and time
              </h2>

              {date && <Calendar value={date} onChange={setDate} locale="en" />}
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-slate-200">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Photo, name and working hours for an easier choice.
                  </p>
                </div>
                ...
                <div className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {(["any", "daniel", "elitsa"] as TherapistKey[]).map((key) => {
                    ...
                    const firstFreeLabel = firstFreeLoading
                      ...
                      : firstFree
                      ? `First available: ${firstFree} on ${selectedDateLabel}`
                      : `No available times on ${selectedDateLabel}`;
                    const subtitle =
                      key === "any"
                        ? "Choose by availability"
                        : item.start && item.end
                        ? `Working hours: ${item.start}–${item.end}`
                        : "";
                            ...
                            {active && key !== "any" && (
                              <div className="package-inline-card mt-3 overflow-hidden rounded-2xl border border-cyan-200/80 bg-gradient-to-r from-slate-950 via-slate-900 to-cyan-950 px-3 py-3 text-left text-white shadow-[0_16px_35px_rgba(14,116,144,0.18)]">
                                ...
                                <div className="mt-2 rounded-xl border border-white/10 bg-black/10 px-3 py-2 text-[11px] leading-5 text-slate-200">
                                  The package is purchased on site and is valid for 6 months.
                                </div>
                                <div className="sr-only">
                                  Shown when a therapist is selected and presents the package price.
                                </div>
                              </div>
                            )}
                          </div>
                ...
                {therapist !== "any" && (
                  <div className="px-4 pb-4">
                    <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900">
                      Selected терапевт: <strong>{t.name}</strong>
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <div className="inline-flex rounded-full border border-slate-300 bg-white p-1 shadow-sm">
                  ...
                  <button
                    ...
                    title={
                      !hourAvailable
                        ? "No available 60-minute interval for this date"
                        : ""
                    }
                  ...
                  <button
                    ...
                    title={
                      !ninetyAvailable
                        ? "No available 90-minute interval for this date"
                        : ""
                    }
        ...
        {selectedTime && (
          <div className="mt-6 rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200">
              <h2 className="text-center text-[22px] font-semibold text-slate-900">
                Попълнете формата, за да bookте час
              </h2>
              <div className="mt-2 text-center text-sm text-slate-600">
                {date ? fmtDateHeader(date, "en") : ""} • {selectedTime} • {duration}{" "}
                min {therapist !== "any" ? `• ${t.name}` : ""}
              </div>
            </div>
            ...
            <form onSubmit={submit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe the pain, location/radiation, when it started, what makes it worse or easier, previous injuries/tests, and your goal..."
                  value={form.symptoms || ""}
                  onChange={(e) =>
